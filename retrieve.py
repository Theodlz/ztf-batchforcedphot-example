...
def get_lightcurve_url(row):
    if row['lightcurve'] is None:
        return None
    else:
        return f"https://ztfweb.ipac.caltech.edu{row['lightcurve']}"
...
def retrieve():
    ...
    # the result (r.text) is an html webpage, which contains a table with the following columns:
    # 1. reqID
    # 2. ra
    # 3. dec
    # 4. startJD
    ...
    df_result['lightcurve'] = df_result.apply(get_lightcurve_url, axis=1)

    # read the files that contains the RA, Dec positions used for the query
    ...
    # if there is less rows than RA, Dec pairs, display a warning
    if len(df_result) < len(ra):
        print(f"Warning: only {len(df_result)} rows found in the ZTF Batch Forced Photometry database, while {len(ra)} RA, Dec pairs were used for the query.\n")
